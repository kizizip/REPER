...
public interface RecipeSearchRepository extends ElasticsearchRepository<RecipeDocument, Long> {
    ...
    // 가게별 레시피에서 재료 포함 검색
    List<RecipeDocument> findByStoreIdAndIngredientsContaining(Long storeId, String ingredient);

    // 재료 미포함 검색
    List<RecipeDocument> findByStoreIdAndIngredientsIsNotContaining(Long storeId, String ingredient);

    // 가게별 레시피 검색
    List<RecipeDocument> findByStoreIdAndRecipeNameContaining(Long storeId, String keyword);
